...
$(function(){
					  ...
					  <div class="d-flex">
					    ...
					    <div class="comment_wrap">
					      <div class="comment_info">
					        <span class="userName badge rounded-pill bg-info mr-2">${elem.replierNickName}</span>
					        <span class="badge bg-success">${elem.replyDate}</span>
					      </div>
					      <div class="comment_content py-2">${elem.reply}</div>
					    </div>
					  </div>`
				if(memberId==elem.replier || auth.includes('ROLE_ADMIN')) {
					replyList += `
	...
	$('.submit button').click(function(){
		let reply = { // 입력 데이터 객체  
			bno : bnoValue, // 게시물 번호 
			reply : $('.replyContent').val(), // 내용 replyContent의 값
			replier : $('.replier').val(),	// 작성자 replier 의 값
			replierNickName : $('.replierNickName').html() // 작성자 닉
		}
	...
	$('.chat').on('click','.reply_modify a',function(e){
		...
		let replier = $(this).closest('li').find('.userName').text();
		
		if(replier!=memberId && !auth.includes('ROLE_ADMIN')){
			return;
		}
		...
		if(operation=='modify'){ // 수정처리 
			...
			replyService.get(rno,function(data){
				...
				replyUpdateForm.find('.replier').html(data.replier);
			})	
